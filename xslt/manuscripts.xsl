...
<xsl:template match="witness">
  ...
  <ul>
    ...
    <xsl:if test="@filename != 'none'">
      ...
      <a href="/ms/{substring-before(@filename, '.')}">
	http://www.cursus.uea.ac.uk/ms/<xsl:value-of select="substring-before(@filename, '.')"/>
      </a>
      </li>
      <li><span class="heading">Range of Manuscript Edited:</span><xsl:text>  </xsl:text> 
      <xsl:variable name="firstcode" select="document(concat('../content/mss/', @filename))//*[@code][1]/@code"/>
      <xsl:variable name="lastcode" select="document(concat('../content/mss/', @filename))//*[@code][position()=last()]/@code"/>
